...
txt(40,54,"Sits inside a standing crop. Nothing stands above 890 mm: the microphone and the 4G antenna are both inside the box. All dimensions in mm.",11.5,MID)
line(40,66,1300,66,LIGHT,1)
line(672,86,672,1095,LIGHT,1)
...
pw=60*S
rect(mx(0)-pw/2,my(940),pw,(940+600)*S,"#ffffff",INK,1.6)
...
circ(bx+bd/2, my(560)+3, 5, "#ffffff", GREEN, 1.8)
poly([(bx+bd/2-9,my(560)-4),(bx+bd/2+9,my(560)-4),(bx+bd/2,my(560)+14)],"none",GREEN,1.3)
...
for (px,py,lab,ly) in [
 ...
 (bx+bd/2,my(560)+3,"microphone port: 5 mm hole, acoustic",206),
 (None,None,"membrane behind it, foam hood over it",220),
 (bx+bd/2,my(430),"4G stick radiates through the plastic wall.",262),
 (None,None,"No external antenna, no mast",276),
 (bx+bd/2,by+bh/2,"IP65 box 300 x 200 x 130, base 300 up",318),
 (mx(0),my(140),"60 x 60 post, 600 driven. No concrete",354),
...
txt(fx(560),fy(890)+4,"nothing above this",9.5,MID,"start")
...
for k,t in enumerate(["Power: survivable. December is the tight month and also the bare-field",
 ...
 "Sound: a capsule at 300 mm behind a membrane hears a smaller circle than",
...
txt(700,900,"D   Probe trench, section",12.5,INK,"start","600")
...
line(40,1115,1300,1115,LIGHT,1)
txt(40,1135,"Theft: matt green throughout, no reflective labels, no branding, cable lock through post and enclosure lugs. Machinery: a box this low is invisible to a sprayer too, so",10,MID)
txt(40,1151,"log the position on the farmer's own GPS and site it on a headland or beside a tramline rather than mid-crop.",10,MID)
A('</svg>')
