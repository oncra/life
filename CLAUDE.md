...
- **Current pickup point**: the kit is `content/kit.md` (Life node v1, one solar 4G box, ~€613) with `kit/order-list.csv` and node software in `node/`. `kit/order-list.csv` is the source of truth for parts and prices; the table on the kit page describes it and must be changed with it.
- **Parts for node 1 were ordered on 2026-09-16**: Berrybase 1070372, reichelt I-653887, UUGear 14639, TelecomShop ORD546258919. Still to order: offgridtec panel and battery, the 1NCE SIM, and the generic items. `node/provision.sh` has still never run on real hardware and the Modbus register maps are from datasheets, so the bench session is the next real step. Handoff: `~/code/sapience-os/briefs/2026-09-13-life-kit-handoff.md`.
