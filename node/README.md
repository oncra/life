...
- `life-push` (= `clients/birdnet-pi-push.py`) posts new detections hourly to `/api/v1/ingest/sound`, and carries the node's heartbeat on the same post.
- `life-heartbeat.py` builds that heartbeat: uptime, CPU temperature, free space on `/data`, unsent queue length, and the cell the 4G stick is camped on (PLMN and cell id from the stick's own HiLink web API). The oracle keeps the first cell heard as the node's home cell and opens a **moved** alert when a later heartbeat comes from another one, and a **silent** alert when a node that has been heard goes quiet for 36 hours. That is the theft answer: no coin-cell tracker, no extra hardware, nothing that chirps. A stolen box that is powered anywhere reports the cell it landed in on its next scheduled wake.
- `firmware/life-guard/` is the tamper guard, an ATtiny84 on the Witty Pi's always-on 3V3 pin with two reed contacts (lid, panel), a tilt switch and a 12 V piezo siren; `life-guard.py` is the Pi's side of it. The lid gets a sixty-second entry delay with chirps while the Pi boots and asks the oracle; panel and tilt get the siren at once. The guard is silenced only by a pulse from the Pi, and the Pi only sends it with a maintenance window in hand, from the oracle's answer or from a window cached at an earlier heartbeat. `life-guard.service` runs `life-guard check` first thing at every boot; `life-push` feeds each heartbeat answer to `life-guard apply`. `GUARD=1` in the env when the board is fitted.
...
2. *Built.* **Heartbeat with metrics**, hourly on the detections post (`life-heartbeat.py`): uptime, CPU temperature, free disk, unsent count, throttle flags, and the serving cell. Battery voltage waits for the VE.Direct cable (v1.1). Silent and struggling are different failures, and so is moved.
