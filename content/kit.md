...
summary: "Life node v1: a single solar-powered 4G box that hears (BirdNET), reads two wired soil probes and posts to the oracle. No farm network, no gateway, no WiFi. About €840 in parts, tamper guard included. Order list with suppliers and direct order links, prices and stock checked 2026-09-24, after the first order for the computer and probes fell through."
order: 3
---
...
| 22 | Tamper guard | ATtiny84 on the Witty Pi's always-on 3V3, two reed contacts (lid, panel) and a tilt switch on a normally-closed loop, a 12 V piezo siren (100 dB) on the load output, a green LED through the wall. Under 1 µA in standby. Firmware and pinout in `node/firmware/life-guard` | reichelt | ~€22 |
...
**Theft is answered in software first, and then with a siren.** The node's hourly heartbeat carries the cell its 4G stick is camped on, read from the stick's own web API, and the oracle remembers the first cell it hears as the node's home. A heartbeat from another cell raises a *moved* alert; a node that has been heard and then stays quiet for 36 hours raises a *silent* alert. A stolen box is a complete, self-powered thing: whoever has it, it wakes on its schedule and reports the cell it landed in, and a cell id on a public map is a few hundred metres. There is nothing in the box for a thief to find. Bluetooth trackers were looked at and left out of the standard kit: an AirTag lasts about a year on its coin cell, announces itself to the phone of whoever carries it after eight to twenty-four hours, and would need its battery changed ten times over the life of the box. On a site with a theft history the owner may drop one in the enclosure as an extra, on the understanding that its battery is theirs to change. The order list carries it as an option at €29.

**The guard** (row 22) is what makes the box unpleasant to take. An ATtiny84 sits on the Witty Pi's always-on 3.3 V and sleeps between contact changes, so the whole thing costs under a microampere, about 0.01 Wh a day against a parasitic floor of 4.6. Three contacts hang on it in a normally-closed loop, so a cut wire counts as an opening: a reed in the lid, a reed on the panel bracket with its magnet glued to the panel frame, and a 45° tilt switch for the post being cut or felled. The lid gets an entry delay the way a house alarm does: a short chirp every three seconds while the Pi boots and asks the oracle, then silence if the steward declared a maintenance window, the siren after sixty seconds if not. The panel contact and the tilt switch skip the delay, because nobody lifts a panel on a whim. The siren is a 100 dB 12 V piezo on the load output through its own fuse: twenty minutes continuous, then a chirp every five seconds until it is answered, so a false alarm nobody hears costs a night's noise and not the battery. Silencing is the oracle's decision: the guard only stops on a pulse from the Pi, and the Pi only sends it with a window in hand, either from the oracle's answer or from a window it cached at an earlier heartbeat. Set the window the day before and the node is in bypass when you arrive, green LED, no chirp. Someone who opens the lid and pulls the battery within forty seconds beats the siren but not the report: the alarm boot posts the loop and the cell before anything else. The panel gets shear nuts or security-Torx through-bolts on top, so the contact opens while the wrong tool is still on the first bolt.
...
13. **The guard has never seen a field.** The entry delay, the 45° tilt threshold and the reed gaps are chosen at a desk. Frost heave, a panel bracket that works loose, a lid that is not quite closed after a visit: each is a false alarm on a farm at night, and each one teaches the farmer to ignore the next. The bench (stage 3.7 of the build plan) and the soak are where the thresholds get set, and a guard that cries twice in the first month gets its delay lengthened before it gets its siren back.
7. **A hung node is silent.** Nothing recovers a wedged Pi on its own. *Half closed:* the heartbeat now exists and rides on the hourly post with uptime, CPU temperature, free disk, unsent count, throttle flags and the serving cell, and the oracle raises *silent* after 36 hours and *moved* on a cell change. Battery voltage waits for the VE.Direct cable. The hardware watchdog is still to be enabled, and a silent alert still means a drive out until it is.
...
`kit/order-list.csv` in the repository carries every line with a direct order link, a price and the date it was checked, grouped into shop baskets so the parts arrive in as few parcels as possible. **reichelt**, four lines and about €193: enclosure, five glands, charge controller and the battery, each with its reichelt product number so they go in through the Direct order form in one pass. **Opencircuit** in the Netherlands, four lines and about €131, free shipping: the Pi 4 2 GB (from an external warehouse, three to five working days), the CH343G RS485 adapter, the 64 GB high-endurance card and the stacking header. **DigiKey** in the Netherlands, one line: three SEN0600 probes at €30.04, the third a spare that also clears DigiKey's €75 free-shipping line. **Accuweb**, €85: the panel, shipped free. Then one SIM, one modem, one scheduler, and the generic parts from Amazon.nl or a builders' merchant: the microphone, the USB-to-terminal breakout, standoffs, lacquer, ferrules and pliers, heatshrink and ties, the battery lead and its fuse, paint, tape, a plastic sheet, the microphone hood, a power meter, and the post. Seven shops and a bouwmarkt trip, about €777 in parts and roughly €20 in postage. Berrybase is no longer in the list: it cancelled order 1070372 rather than add a VAT number after checkout, and its Pi and probe stock went with it. Rows marked `alt` are the parts considered and not taken, each with the reason, including the 10 m solar cable kit that would have been bought to use 30 cm of it.
