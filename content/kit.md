...
Two open projects cover parts of this node, and one of them was read properly rather than skimmed.

**[acoupi](https://github.com/acoupi/acoupi) (GPL-3.0) is a well-built version of the node software we wrote by hand.** Reading the source rather than the README: it has a persistent SQLite message store that tracks which messages are unsent and records the server's response, a send task that takes `max_messages` and an oldest-first ordering, an `HTTPMessenger` that posts JSON to a base URL with configurable headers, a `Heartbeat` carrying device id, status and device metrics, and it depends on `astral`, so it computes dawn and dusk itself. Those are, in order, our offline queue, our hourly batching, our ingest client, the watchdog we do not have, and the sunrise-relative schedule we were going to rewrite nightly with a cron job. The `Model` protocol a classifier has to satisfy is a single method, `run(recording) -> ModelOutput`.

We are not rebuilding on it yet, for one architectural reason found in the code. acoupi runs its tasks on **Celery, with a default broker of `pyamqp://guest@localhost//`**, which is RabbitMQ: an Erlang virtual machine, tens of megabytes of memory, and a cold start to pay on every boot. That is a reasonable design for a device that stays powered. This node boots, works for an hour in midwinter, and shuts down again, so it would pay the broker's startup cost twice a day for a schedule that a systemd timer already runs for nothing. There is also a licence decision to make deliberately rather than by accident: acoupi is GPL-3.0 and this repository's code is Apache-2.0. Apache code can be taken into a GPL work but not the reverse, so building the node on acoupi makes the node software GPL-3.0, while the oracle stays Apache-2.0 on the other side of an HTTP boundary. Workable, common, and not something to discover after the fact.

What we take from it now is the design, not the dependency: the message store belongs in SQLite on the writable partition rather than a JSONL file, the heartbeat should carry metrics and not just a timestamp, and dawn and dusk should be computed on the node with `astral` instead of rewriting an absolute-time schedule every night.

**[`acoupi_birdnet`](https://github.com/acoupi/acoupi_birdnet) is stale, and that is an opening rather than a problem.** It is MIT-licensed, last touched in January 2025, pins `acoupi>=0.3.0` against a core now at 0.8.0, and caps Python at 3.13 where the core allows 3.15. But the integration point it uses, `DetectionProgram` in `acoupi.programs.templates`, is still exported in 0.8.0, and the model interface is that one method. Refreshing it is a small job in the one open framework aimed exactly at this problem, and it is worth doing as a contribution rather than keeping a private equivalent.

**[Bugg](https://github.com/bugg-resources)** publishes its full hardware design, PCBs and enclosure CAD included, under a non-commercial licence, with 97 units in the field across Europe under [TABMON](https://besjournals.onlinelibrary.wiley.com/doi/10.1111/2041-210x.70308). We are not building this node because nobody has built one. We are building it because the ones that exist either upload raw audio, or cannot be used commercially, or are closed. The full reasoning is in [Prior art](/docs/prior-art).
