...
Before the next node is ordered, two things found in the open-source scan are worth acting on. **[acoupi](https://github.com/acoupi/acoupi)** is a GPL-3.0 framework that already does what our node agent does by hand: record, classify on the edge, manage the backlog, send to a server. **BirdNET-Go**, which we already run, publishes detections over MQTT and webhooks and exposes a Prometheus endpoint, which may make our own push client redundant. And **[Bugg](https://github.com/bugg-resources)** publishes its full hardware design, PCBs, enclosure CAD and all, under a non-commercial licence, with 97 units in the field across Europe under [TABMON](https://besjournals.onlinelibrary.wiley.com/doi/10.1111/2041-210x.70308). We are not building this node because nobody has built one. We are building it because the ones that exist either upload raw audio, or cannot be used commercially, or are closed. The full reasoning is in [Prior art](/docs/prior-art).
